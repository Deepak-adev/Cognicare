...
const CaregiverHomeTab = () => {
  ...
  if (!patient) return <View style={globalStyles.container}><Text>{t('loading') || 'Loading...'}</Text></View>;

  return (
    <ScrollView style={globalStyles.container} showsVerticalScrollIndicator={false}>
      <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24, marginTop: 10 }}>
        <View>
          <Text style={{ fontSize: 18, color: colors.textMuted, fontWeight: '600', marginBottom: 4 }}>{t('Caregiver View') || 'Caregiver View'}</Text>
        ...
        {isOffline && (
          <View style={{ backgroundColor: colors.danger + '20', padding: 8, borderRadius: 12 }}>
            <Text style={{ color: colors.danger, fontWeight: 'bold' }}>{t('Offline') || 'Offline'}</Text>
          </View>
        )}
      </View>
        ...
        <View style={{ backgroundColor: sundowningRiskWindow ? '#991b1b' : colors.danger, padding: 16, borderRadius: 16, marginBottom: 20 }}>
          <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
            <View style={{ flexDirection: 'row', alignItems: 'center' }}>
              ...
              <Text style={{ color: '#ffffff', fontWeight: '900', fontSize: 18 }}>{t("URGENT: PATIENT LEFT SAFE ZONE") || "URGENT: PATIENT LEFT SAFE ZONE"}</Text>
            </View>
            <TouchableOpacity onPress={() => triggerRescueProtocol(false)} style={{ backgroundColor: '#ffffff30', paddingHorizontal: 12, paddingVertical: 6, borderRadius: 12 }}>
              <Text style={{ color: '#ffffff', fontWeight: '800', fontSize: 14 }}>{t("Dismiss") || "Dismiss"}</Text>
            </TouchableOpacity>
          </View>
      ...
      {patient.appointments && patient.appointments.length > 0 && (
        <Card>
          <View style={styles.cardHeader}>
            ...
            <Text style={styles.cardTitle}>{t('Upcoming Appointments') || 'Upcoming Appointments'}</Text>
          </View>
          {patient.appointments.map(app => (
            <View key={app.id} style={styles.row}>
              ...
              <View style={{ backgroundColor: colors.primaryLight + '20', paddingHorizontal: 12, paddingVertical: 6, borderRadius: 12, alignSelf: 'flex-start' }}>
                <Text style={{ color: colors.primary, fontWeight: '700' }}>{t("Manage") || "Manage"}</Text>
              </View>
            </View>
          ))}
        </Card>
    ...
    <Card style={{ backgroundColor: '#ffffff', borderColor: '#e2e8f0' }}>
      ...
      <View style={{ alignItems: 'center', marginBottom: 24 }}>
        ...
        <View style={{ flexDirection: 'row', marginTop: 8, alignItems: 'center', gap: 16 }}>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
            ...
            <Text style={{ fontSize: 12, color: colors.textMuted, fontWeight: '600' }}>{t("Current Performance") || "Current Performance"}</Text>
          </View>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
            <View style={{ width: 12, height: 12, borderColor: '#94a3b8', borderWidth: 2, borderStyle: 'dashed', borderRadius: 6 }} />
            <Text style={{ fontSize: 12, color: colors.textMuted, fontWeight: '600' }}>{t("Baseline") || "Baseline"}</Text>
          </View>
        </View>
      </View>
      ...
      <View style={{ marginBottom: 24 }}>
        <View style={{ flexDirection: 'row', borderBottomWidth: 1, borderBottomColor: '#e2e8f0', paddingBottom: 8, marginBottom: 8 }}>
          <Text style={{ flex: 2, fontWeight: '700', color: colors.textMain }}>{t("Domain") || "Domain"}</Text>
          <Text style={{ flex: 1, fontWeight: '700', color: colors.textMain, textAlign: 'right' }}>{t("Performance") || "Performance"}</Text>
          <Text style={{ flex: 1.5, fontWeight: '700', color: colors.textMain, textAlign: 'right' }}>{t("Status") || "Status"}</Text>
        </View>
      ...
      <View style={{ marginBottom: 24, backgroundColor: '#f8fafc', padding: 16, borderRadius: 16, borderWidth: 1, borderColor: '#f1f5f9' }}>
        <View style={{ flexDirection: 'row', alignItems: 'flex-start', marginBottom: 12 }}>
          ...
          <Text style={{ fontSize: 16, color: colors.textMain, flex: 1, lineHeight: 24 }}>
            <Text style={{ fontWeight: '800' }}>{t("Improvement Needed:") || "Improvement Needed:"}</Text>
          ...
          if (worstTrend.diff < 0) {
            return (
              <View style={{ flexDirection: 'row', alignItems: 'flex-start', marginBottom: 12 }}>
                ...
                <Text style={{ fontSize: 16, color: colors.textMain, flex: 1, lineHeight: 24 }}>
                  <Text style={{ fontWeight: '800' }}>{t("Trend:") || "Trend:"}</Text>
        ...
        <View style={{ flexDirection: 'row', alignItems: 'flex-start', marginBottom: 12 }}>
          ...
          <Text style={{ fontSize: 16, color: colors.textMain, flex: 1, lineHeight: 24 }}>
            <Text style={{ fontWeight: '800' }}>{t("Risk Zone:") || "Risk Zone:"}</Text>
        ...
        <View style={{ flexDirection: 'row', alignItems: 'flex-start' }}>
          ...
          <Text style={{ fontSize: 16, color: colors.textMain, flex: 1, lineHeight: 24 }}>
            <Text style={{ fontWeight: '800' }}>{t("AI Action:") || "AI Action:"}</Text>
      ...
      <Text style={{ fontSize: 14, color: colors.textMuted, lineHeight: 22, fontWeight: '500', marginBottom: 40 }}>
        <Text style={{ fontWeight: '800', color: colors.textMain }}>{t("Important:") || "Important:"}</Text> The "risk zone" should be presented as a <Text style={{ fontWeight: '800', color: colors.textMain }}>{t("support/attention indicator, not a dementia diagnosis") || "support/attention indicator, not a dementia diagnosis"}</Text>; persistent concerning changes can be flagged for caregiver/clinician review.
      </Text>
    </Card>
  );
};
...
const CaregiverHealthTab = () => {
  ...
  if (!cognitiveProfile) return <View style={globalStyles.container}><Text>{t("Loading...") || "Loading..."}</Text></View>;
    ...
    <ScrollView style={globalStyles.container} showsVerticalScrollIndicator={false}>
      ...
      <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 16 }}>
        <Card style={[styles.statBox, { flex: 1, marginRight: 8, padding: 16 }]}>
          ...
          <Text style={styles.statBoxLabel}>{t("Games Played") || "Games Played"}</Text>
        ...
        <Card style={[styles.statBox, { flex: 1, marginHorizontal: 4, padding: 16 }]}>
          ...
          <Text style={styles.statBoxLabel}>{t("Current Streak") || "Current Streak"}</Text>
          <Text style={styles.statBoxValue}>{activityStats?.currentStreak || 0} <Text style={{ fontSize: 14 }}>{t("days") || "days"}</Text></Text>
        </Card>
        <Card style={[styles.statBox, { flex: 1, marginLeft: 8, padding: 16 }]}>
          <CalendarDays color={colors.success} size={24} style={{ marginBottom: 8 }} />
          <Text style={styles.statBoxLabel}>{t("Active Days") || "Active Days"}</Text>
      ...
      <Card>
        <View style={styles.cardHeader}>
          ...
          <Text style={styles.cardTitle}>{t("Cognitive Progress") || "Cognitive Progress"}</Text>
        </View>
        
        <Text style={{ fontSize: 18, fontWeight: '700', color: colors.textMain, marginBottom: 16 }}>{t("Memory Performance (This Week)") || "Memory Performance (This Week)"}</Text>
        ...
        <View style={{ marginTop: 16, backgroundColor: colors.bgSubtle, padding: 16, borderRadius: 16 }}>
          <Text style={{ fontSize: 16, color: colors.textMain, fontWeight: '600', marginBottom: 12, lineHeight: 24 }}>{t("✅ A significant change in cognitive-task performance was observed.") || "✅ A significant change in cognitive-task performance was observed."}</Text>
          <View style={{ flexDirection: 'row', alignItems: 'center', backgroundColor: colors.accent + '20', alignSelf: 'flex-start', paddingHorizontal: 12, paddingVertical: 8, borderRadius: 8 }}>
            ...
            <Text style={{ color: colors.accentDark || '#b45309', fontWeight: '800', fontSize: 16 }}>{t("⚠ Review Recommended") || "⚠ Review Recommended"}</Text>
          </View>
        </View>
      </Card>
      ...
      <Card>
        <View style={styles.cardHeader}>
          ...
          <Text style={styles.cardTitle}>{t("Cognitive Fingerprint") || "Cognitive Fingerprint"}</Text>
        </View>
        <Text style={{ color: colors.textMuted, marginBottom: 16 }}>{t("AI analysis of how the patient performs beyond flat scores.") || "AI analysis of how the patient performs beyond flat scores."}</Text>
      ...
      <Card>
        <View style={styles.cardHeader}>
          ...
          <Text style={styles.cardTitle}>{t("Cognitive Domains") || "Cognitive Domains"}</Text>
        </View>
      ...
      <Card>
        <View style={styles.cardHeader}>
          <Activity color={colors.primary} size={28} />
          <Text style={styles.cardTitle}>{t("Clinical Biomarkers") || "Clinical Biomarkers"}</Text>
        </View>
        <Text style={{ color: colors.textMuted, marginBottom: 16 }}>{t("Data extracted from digitial clinical assessments (Clock Drawing, Verbal Fluency).") || "Data extracted from digitial clinical assessments (Clock Drawing, Verbal Fluency)."}</Text>
        ...
        {clinicalTests && clinicalTests.map((test) => (
          <View key={test.id} style={{ marginBottom: 20, backgroundColor: '#f8fafc', padding: 16, borderRadius: 16, borderWidth: 1, borderColor: '#e2e8f0' }}>
            ...
            {test.type === 'Clock Drawing' && (
              <View>
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 8 }}>
                  <Text style={{ color: colors.textMain, fontWeight: '600' }}>{t("Time Taken:") || "Time Taken:"}</Text>
                  <Text style={{ color: colors.textMain, fontWeight: '800' }}>{test.data.timeTaken} sec</Text>
                </View>
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 8 }}>
                  <Text style={{ color: colors.textMain, fontWeight: '600' }}>{t("Total Strokes:") || "Total Strokes:"}</Text>
                  <Text style={{ color: colors.textMain, fontWeight: '800' }}>{test.data.strokes}</Text>
                </View>
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 12 }}>
                  <Text style={{ color: colors.textMain, fontWeight: '600' }}>{t("Hesitation Pauses:") || "Hesitation Pauses:"}</Text>
            ...
            {test.type === 'Verbal Fluency' && (
              <View>
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 8 }}>
                  <Text style={{ color: colors.textMain, fontWeight: '600' }}>{t("Score (Unique Words):") || "Score (Unique Words):"}</Text>
                  <Text style={{ color: colors.primary, fontWeight: '900', fontSize: 18 }}>{test.data.score}</Text>
                </View>
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 12 }}>
                  <Text style={{ color: colors.textMain, fontWeight: '600' }}>{t("Category:") || "Category:"}</Text>
        ...
        {(!clinicalTests || clinicalTests.length === 0) && (
          <Text style={{ color: colors.textMuted, textAlign: 'center' }}>{t("No clinical test data available yet.") || "No clinical test data available yet."}</Text>
        )}
      </Card>
      ...
      <Card>
        <View style={styles.cardHeader}>
          ...
          <Text style={styles.cardTitle}>{t("Recent Activity") || "Recent Activity"}</Text>
        </View>
    ...
    <ScrollView style={globalStyles.container} showsVerticalScrollIndicator={false}>
      ...
      <Card style={{ backgroundColor: '#f0f9ff', borderColor: '#e0f2fe', borderWidth: 1 }}>
        <View style={[styles.cardHeader, { justifyContent: 'space-between', marginBottom: 12 }]}>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 16 }}>
            ...
            <Text style={styles.cardTitle}>{t("Personal Memory Profile") || "Personal Memory Profile"}</Text>
          </View>
        ...
        <Text style={{ color: colors.textMuted, marginBottom: 16 }}>{t("Automatically extracted during patient voice onboarding. Verify and fill missing details.") || "Automatically extracted during patient voice onboarding. Verify and fill missing details."}</Text>
        ...
        <View style={{ backgroundColor: '#ffffff', borderRadius: 16, padding: 16, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.05, shadowRadius: 8, elevation: 2 }}>
          <View style={{ flexDirection: 'row', marginBottom: 12 }}>
            <Text style={{ fontWeight: '800', width: 90, color: colors.textMain }}>{t("Name:") || "Name:"}</Text>
            <Text style={{ color: colors.textMuted, flex: 1 }}>{patient?.name || 'Unknown'}</Text>
          </View>
          <View style={{ flexDirection: 'row', marginBottom: 12 }}>
            <Text style={{ fontWeight: '800', width: 90, color: colors.textMain }}>{t("Age:") || "Age:"}</Text>
            <Text style={{ color: colors.textMuted, flex: 1 }}>{patient?.age || 'Not provided'}</Text>
          </View>
          <View style={{ flexDirection: 'row', marginBottom: 12 }}>
            <Text style={{ fontWeight: '800', width: 90, color: colors.textMain }}>{t("Interests:") || "Interests:"}</Text>
          ...
          <View style={{ flexDirection: 'row', marginBottom: 12 }}>
            <Text style={{ fontWeight: '800', width: 90, color: colors.textMain }}>{t("Family:") || "Family:"}</Text>
          ...
          <View style={{ flexDirection: 'row' }}>
            <Text style={{ fontWeight: '800', width: 90, color: colors.textMain }}>{t("Location:") || "Location:"}</Text>
      ...
      <Card>
        <View style={styles.cardHeader}>
          ...
          <Text style={styles.cardTitle}>{t("AI Insights") || "AI Insights"}</Text>
        </View>
        
        {insights.length === 0 ? (
          <Text style={{ color: colors.textMuted, fontSize: 16 }}>{t("No significant changes detected recently.") || "No significant changes detected recently."}</Text>
        ) : (
      ...
      <Modal visible={isEditingProfile} transparent animationType="fade">
        <View style={styles.modalOverlay}>
          <View style={[styles.modalContent, { width: '90%' }]}>
            <Text style={styles.modalTitle}>{t("Edit Memory Profile") || "Edit Memory Profile"}</Text>
            <Text style={{ color: colors.textMuted, marginBottom: 16 }}>{t("Fill in details the AI missed.") || "Fill in details the AI missed."}</Text>
            
            <Text style={styles.inputLabel}>{t("Patient Name") || "Patient Name"}</Text>
            <TextInput 
            ...
            <Text style={styles.inputLabel}>{t("Location (City, State)") || "Location (City, State)"}</Text>
            <TextInput 
              style={styles.modalInput} 
            ...
            <Text style={styles.inputLabel}>{t("Interests (comma separated)") || "Interests (comma separated)"}</Text>
            <TextInput 
              style={styles.modalInput} 
            ...
            <Text style={styles.inputLabel}>{t("Family (comma separated)") || "Family (comma separated)"}</Text>
            <TextInput 
              style={styles.modalInput} 
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginTop: 12, width: '100%' }}>
              <TouchableOpacity onPress={() => setIsEditingProfile(false)} style={[styles.modalBtn, { backgroundColor: '#f1f5f9', flex: 1, marginRight: 8 }]}>
                <Text style={{ color: '#64748b', fontWeight: '700', fontSize: 16, textAlign: 'center' }}>{t("Cancel") || "Cancel"}</Text>
              </TouchableOpacity>
              <TouchableOpacity onPress={handleSaveProfile} style={[styles.modalBtn, { backgroundColor: colors.primary, flex: 1, marginLeft: 8 }]}>
                <Text style={{ color: '#ffffff', fontWeight: '700', fontSize: 16, textAlign: 'center' }}>{t("Save Details") || "Save Details"}</Text>
              </TouchableOpacity>
            </View>
          </View>
        </View>
    ...
    <ScrollView style={globalStyles.container} showsVerticalScrollIndicator={false}>
      <Card>
        <View style={styles.cardHeader}>
          ...
          <Text style={styles.cardTitle}>{t('App Personalization') || 'App Personalization'}</Text>
        </View>
        <Text style={{ color: colors.textMuted, fontSize: 16, marginBottom: 20 }}>{t('Remotely configure the patient\'s device experience.') || 'Remotely configure the patient\'s device experience.'}</Text>
        
        {/* Language */}
        <View style={styles.settingBlock}>
          <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 12 }}>
             <Globe color={colors.textMain} size={24} style={{ marginRight: 8 }} />
             <Text style={[styles.settingTitle, { marginBottom: 0 }]}>{t('Language') || 'Language'}</Text>
          </View>
        ...
        <View style={styles.settingBlock}>
          <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 12 }}>
             ...
             <Text style={[styles.settingTitle, { marginBottom: 0 }]}>{t('Font Size') || 'Font Size'}</Text>
          </View>
        ...
        <View style={{ flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'space-between' }}>
          ...
          >
            ...
            <Text style={[styles.gridSettingTitle, patientSettings.highContrast && { color: '#ffffff' }]}>{t('High Contrast') || 'High Contrast'}</Text>
          </TouchableOpacity>
          ...
          >
            ...
            <Text style={[styles.gridSettingTitle, patientSettings.colorBlindMode && { color: '#ffffff' }]}>{t('Color Blind Mode') || 'Color Blind Mode'}</Text>
          </TouchableOpacity>
          ...
          >
            ...
            <Text style={[styles.gridSettingTitle, patientSettings.voiceFeedback && { color: '#ffffff' }]}>{t('Voice Feedback') || 'Voice Feedback'}</Text>
          </TouchableOpacity>
          ...
          >
            ...
            <Text style={[styles.gridSettingTitle, patientSettings.simplifiedUI && { color: '#ffffff' }]}>{t('Simplified UI') || 'Simplified UI'}</Text>
          </TouchableOpacity>
          ...
          >
            <View style={{ flexDirection: 'row', alignItems: 'center' }}>
              ...
              <View>
                <Text style={[styles.gridSettingTitle, patientSettings.ignoreAccidentalTaps && { color: '#ffffff' }]}>{t('Ignore Accidental Taps') || 'Ignore Accidental Taps'}</Text>
                <Text style={{ color: patientSettings.ignoreAccidentalTaps ? '#e2e8f0' : colors.textMuted, fontSize: 13, marginTop: 4 }}>{t('Helps with hand tremors') || 'Helps with hand tremors'}</Text>
              </View>
            </View>
          </TouchableOpacity>
        </View>
    ...
    <ScrollView style={globalStyles.container} showsVerticalScrollIndicator={false}>
      ...
      {routineAlerts && routineAlerts.length > 0 && (
        <View style={{ backgroundColor: '#fff1f2', borderRadius: 24, padding: 24, marginBottom: 20, borderWidth: 1, borderColor: '#fecdd3', shadowColor: '#e11d48', shadowOffset: { width: 0, height: 8 }, shadowOpacity: 0.15, shadowRadius: 16, elevation: 6 }}>
          <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 16 }}>
            ...
            <Text style={{ fontSize: 20, fontWeight: '900', color: '#e11d48' }}>{t("Routine Deviations") || "Routine Deviations"}</Text>
          </View>
      ...
      <Card>
        <View style={styles.cardHeader}>
          ...
          <Text style={styles.cardTitle}>{t("Daily Schedule (My Day)") || "Daily Schedule (My Day)"}</Text>
        </View>
        <Text style={{ color: colors.textMuted, marginBottom: 16 }}>{t("Manage the patient's daily timeline tasks.") || "Manage the patient's daily timeline tasks."}</Text>
        ...
        >
          ...
          <Text style={{ color: '#4f46e5', fontWeight: '800', fontSize: 16 }}>{t("Add New Task") || "Add New Task"}</Text>
        </TouchableOpacity>
      </Card>
      ...
      <Card>
        <View style={styles.cardHeader}>
          ...
          <Text style={styles.cardTitle}>{t("Medication Reminders") || "Medication Reminders"}</Text>
        </View>
        <Text style={{ color: colors.textMuted, marginBottom: 16 }}>{t("Manage the patient's medications. Status is synced instantly.") || "Manage the patient's medications. Status is synced instantly."}</Text>
        ...
        >
          ...
          <Text style={{ color: '#ea580c', fontWeight: '800', fontSize: 16 }}>{t("Add New Medicine") || "Add New Medicine"}</Text>
        </TouchableOpacity>
      </Card>
      ...
      <Modal visible={isAddingTask} transparent animationType="fade">
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <Text style={styles.modalTitle}>{t("Add Daily Task") || "Add Daily Task"}</Text>
            <TextInput 
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginTop: 12, width: '100%' }}>
              <TouchableOpacity onPress={() => setIsAddingTask(false)} style={[styles.modalBtn, { backgroundColor: '#f1f5f9' }]}>
                <Text style={{ color: '#64748b', fontWeight: '700', fontSize: 16 }}>{t("Cancel") || "Cancel"}</Text>
              </TouchableOpacity>
              <TouchableOpacity onPress={handleSaveTask} style={[styles.modalBtn, { backgroundColor: '#4f46e5' }]}>
                <Text style={{ color: '#ffffff', fontWeight: '700', fontSize: 16 }}>{t("Save Task") || "Save Task"}</Text>
              </TouchableOpacity>
            </View>
          </View>
        </View>
      ...
      <Modal visible={isAddingMed} transparent animationType="fade">
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <Text style={styles.modalTitle}>{t("Add Medication") || "Add Medication"}</Text>
            <TextInput 
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginTop: 12, width: '100%' }}>
              ...
              <TouchableOpacity onPress={handleSaveMedication} style={[styles.modalBtn, { backgroundColor: '#ea580c' }]}>
                <Text style={{ color: '#ffffff', fontWeight: '700', fontSize: 16 }}>{t("Save Medicine") || "Save Medicine"}</Text>
              </TouchableOpacity>
            </View>
          </View>
        </View>
